...
public class signUp1_5 extends AppCompatActivity {
    Spinner gender,class_grade;
    EditText guardianMail,yourMail,guardianPhn,password,confirmPassword,name;
    ArrayAdapter<CharSequence> gender_adapter,class_grade_adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sign_up15);
        gender=findViewById(R.id.role1);
        guardianMail=findViewById(R.id.editTextTextPersonName1);
        yourMail=findViewById(R.id.editTextTextPersonName5);guardianPhn=findViewById(R.id.editTextTextPersonName2);
        ...
        button=findViewById(R.id.button);error=findViewById(R.id.errorTV);
        ...
        class_grade_adapter=ArrayAdapter.createFromResource(this,R.array.class_array,R.layout.class_spinner_layout);
    ...
    private void createUser() {
        ...
        String sguardianMail,syourMail,sguardianPhn,spassword,sconfirmPassword,sdob,sgender,sclass,role="Student(1-5)";
        sguardianMail=guardianMail.getText().toString();
        syourMail=yourMail.getText().toString();
        sguardianPhn=guardianPhn.getText().toString();
        ...
        if(TextUtils.isEmpty(sguardianMail)){
        ...
        else if(!TextUtils.isEmpty(sguardianPhn)&&!Patterns.PHONE.matcher(sguardianPhn).matches()){
            guardianPhn.setError("Enter valid email");
            guardianPhn.requestFocus();
        }
        else if(!TextUtils.isEmpty(syourMail)&&!Patterns.EMAIL_ADDRESS.matcher(syourMail).matches()){
        ...
        else if(TextUtils.isEmpty(sguardianPhn)==false&&sguardianPhn.length()!=10){
            guardianPhn.setError("Password cannot be empty");
            guardianPhn.requestFocus();}
            ...
            mAuth.createUserWithEmailAndPassword(sguardianMail,spassword).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                @Override
                public void onComplete(@NonNull Task<AuthResult> task) {
                    if(task.isSuccessful()){
                        UserStudent userStudent =new UserStudent(sname,sguardianMail,syourMail,sguardianPhn,sdob,sgender,sclass,role,null,false);
                        ...
                        .setValue(userStudent)
                                .addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if(task.isSuccessful()){
                                            Toast.makeText(signUp1_5.this, "User signed up successfully", Toast.LENGTH_SHORT).show();
                                            startActivity(new Intent(signUp1_5.this,MainActivity.class));
                                        }else {
                                            Toast.makeText(signUp1_5.this, "Error\t"+task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                                        }
                                    }
                                });
                    }
